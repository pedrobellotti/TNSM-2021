...
#include "applications/udp-ping-helper.h"
...
//Carga alta = 20; Carga media = 10; Carga baixa = 5; Testar = 50

// Balance of client-server pairs in the simulation.
static ns3::GlobalValue
  g_balance ("Balance", "Balance of client-server pairs.",
              ns3::BooleanValue(true),
              ns3::MakeBooleanChecker ());

// Balance of client-server pairs in the simulation.
static ns3::GlobalValue
  g_maxSW ("MaxSW", "Maximum number of software switches.",
              ns3::UintegerValue (1),
              ns3::MakeUintegerChecker<uint16_t> ());
...
main (int argc, char *argv[])
{
  ...
  // Configure the CsmaHelper to connect OpenFlow switches
  CsmaHelper csmaHelper;
  csmaHelper.SetChannelAttribute ("DataRate", DataRateValue (DataRate ("10Gbps"))); //10Gbps
  csmaHelper.SetChannelAttribute ("Delay", TimeValue (MicroSeconds (15)));
  CsmaHelper csmaPingHelper;
  csmaPingHelper.SetChannelAttribute ("DataRate", DataRateValue (DataRate ("10Gbps")));
  csmaPingHelper.SetChannelAttribute ("Delay", TimeValue (MicroSeconds (0)));
  ...
  Ptr<QosCustomController> controllerApp = CreateObject<QosCustomController> ();
  ...
  NodeContainer switchNodesSW;
  UintegerValue max_SW;
  GlobalValue::GetValueByName ("MaxSW", max_SW);
  uint32_t numSW = max_SW.Get ();
  switchNodesSW.Create (numSW);
  /*Ptr<Node> switchNodeSw1 = switchNodes.Get (3);
  Ptr<Node> switchNodeSw2 = switchNodes.Get (4);
  Names::Add ("sw1", switchNodeSw1);
  Names::Add ("sw2", switchNodeSw2); */
  for (size_t i=0; i<numSW; i++)
  {
    Names::Add ("sw"+std::to_string(i+1), switchNodesSW.Get(i));
  }
  ...
  of13Helper->SetDeviceAttribute ("PipelineTables", UintegerValue (1));
  of13Helper->SetDeviceAttribute ("CpuCapacity", StringValue ("2Gbps")); //2Gbps
  of13Helper->SetDeviceAttribute ("FlowTableSize", UintegerValue (1024)); //1024
  of13Helper->SetDeviceAttribute ("TcamDelay", TimeValue (MicroSeconds (20)));
  Ptr<OFSwitch13Device> switchDeviceHw = of13Helper->InstallSwitch (switchNodeHw);

  // Configure switch nodes SW as a software-based OpenFlow switch.
  of13Helper->SetDeviceAttribute ("PipelineTables", UintegerValue (1));
  of13Helper->SetDeviceAttribute ("CpuCapacity", StringValue ("750Mbps")); //750Mbps-1Gbps
  of13Helper->SetDeviceAttribute ("FlowTableSize", UintegerValue (8192)); //8192
  of13Helper->SetDeviceAttribute ("TcamDelay", TimeValue (MicroSeconds (100)));
  OFSwitch13DeviceContainer switchDevicesSW;
  for (size_t i=0; i<numSW; i++)
  {
    switchDevicesSW.Add(of13Helper->InstallSwitch (switchNodesSW.Get(i)));
  }

  // Connecting switches.
  std::vector<uint32_t> ul2swports;
  std::vector<uint32_t> dl2swports;
  ...
  for (size_t i = 0; i<numSW; i++)
  {
    //Não alterar essa ordem! Usado para identificar a direção do tráfego (UL porta 1, DL porta 2)
    NetDeviceContainer sw2ulLink = csmaHelper.Install (switchNodesSW.Get(i), switchNodeUl);
    uint32_t sw2ulPort = switchDevicesSW.Get(i)->AddSwitchPort (sw2ulLink.Get (0))->GetPortNo ();
    uint32_t ul2swPort = switchDeviceUl->AddSwitchPort (sw2ulLink.Get (1))->GetPortNo ();
    ul2swports.push_back(ul2swPort);

    NetDeviceContainer sw2dlLink = csmaHelper.Install (switchNodesSW.Get(i), switchNodeDl);
    uint32_t sw2dlPort = switchDevicesSW.Get(i)->AddSwitchPort (sw2dlLink.Get (0))->GetPortNo ();
    uint32_t dl2swPort = switchDeviceDl->AddSwitchPort (sw2dlLink.Get (1))->GetPortNo ();
    dl2swports.push_back(dl2swPort);
    controllerApp->NotifySwSwitch (switchDevicesSW.Get(i), sw2ulPort, sw2dlPort);
  ...
  //Creating groups
  controllerApp->CreateGroups (switchDeviceUl,ul2swports);
  controllerApp->CreateGroups (switchDeviceDl,dl2swports);
  ...
  // Create the ping server/client
  Ptr<Node> pingServerNode = CreateObject<Node> ();
  Ptr<Node> pingClientNode = CreateObject<Node> ();

  // Create the client and server nodes and setting their names.
  ...
  for (uint32_t i = 0; i < numHosts; i++)
    {
      ...
      name1 << "sv" << i + 1;
      Names::Add (name1.str (), serverNodes.Get (i));
    }

  // Install the TCP/IP stack into hosts nodes.
  InternetStackHelper internet;
  internet.Install(pingClientNode);
  internet.Install(pingServerNode);
  internet.Install (serverNodes);
  internet.Install (clientNodes);

  // Configure IPv4 addresses helper.
  Ipv4AddressHelper ipv4helpr;
  ipv4helpr.SetBase ("10.0.0.0", "255.255.240.0");
  ...
  // Get the balance from global attribute.
  BooleanValue booleanValue;
  GlobalValue::GetValueByName ("Balance", booleanValue);
  bool balance = booleanValue.Get ();
  NS_LOG_INFO ("Balance set to " << balance);
  ...
  //Ping app
  NetDeviceContainer pingServerLink = csmaPingHelper.Install(switchNodeDl, pingServerNode);
  NetDeviceContainer pingClientLink = csmaPingHelper.Install(switchNodeUl, pingClientNode);
  uint32_t dl2svPing = switchDeviceDl->AddSwitchPort (pingServerLink.Get (0))->GetPortNo ();
  uint32_t ul2clPing = switchDeviceUl->AddSwitchPort (pingClientLink.Get (0))->GetPortNo ();
  Ipv4InterfaceContainer pingServerIface = ipv4helpr.Assign (pingServerLink.Get(1));
  Ipv4InterfaceContainer pingClientIface = ipv4helpr.Assign (pingClientLink.Get(1));
  controllerApp->NotifyDl2Sv(dl2svPing, pingServerIface.GetAddress(0));
  controllerApp->NotifyUl2Cl(ul2clPing, pingClientIface.GetAddress(0));
  ...
  for (uint32_t i = 0; i < numHosts; i++)
    {
      ...
      {
        if(i < (numHosts/2)){
          // Connect half of numbers servers node to the DL switch.
          NetDeviceContainer dl2svLink = csmaHelper.Install (switchNodeDl, serverNodes.Get (i));
          uint32_t dl2svPort = switchDeviceDl->AddSwitchPort (dl2svLink.Get (0))->GetPortNo ();
          serverDevices.Add (dl2svLink.Get (1));

          // Assign IP to the server node and notify the controller.
          Ipv4InterfaceContainer tempIpIface1 = ipv4helpr.Assign (dl2svLink.Get (1));
          serverIpIfaces.Add (tempIpIface1);
          controllerApp->NotifyDl2Sv (dl2svPort, tempIpIface1.GetAddress (0));

          // Connect half of numbers client node to the UL switch.
        ...
        { 
          // Connect half of numbers servers node to the UL switch.
          NetDeviceContainer ul2svLink = csmaHelper.Install (switchNodeUl, serverNodes.Get (i));
          uint32_t ul2svPort = switchDeviceUl->AddSwitchPort (ul2svLink.Get (0))->GetPortNo ();
          serverDevices.Add (ul2svLink.Get (1));

          // Assign IP to the server node and notify the controller.
          Ipv4InterfaceContainer tempIpIface1 = ipv4helpr.Assign (ul2svLink.Get (1));
          serverIpIfaces.Add (tempIpIface1);
          controllerApp->NotifyUl2Sv (ul2svPort, tempIpIface1.GetAddress (0));

          // Connect half of numbers client node to the DL switch.
          NetDeviceContainer dl2clLink = csmaHelper.Install (switchNodeDl, clientNodes.Get (i));
          uint32_t dl2clPort = switchDeviceDl->AddSwitchPort (dl2clLink.Get (0))->GetPortNo ();
          clientDevices.Add (dl2clLink.Get (1));

          // Assign IP to the client node and notify the controller.
          Ipv4InterfaceContainer tempIpIface = ipv4helpr.Assign (dl2clLink.Get (1));
          clientIpIfaces.Add (tempIpIface);
          controllerApp->NotifyUl2Cl (dl2clPort, tempIpIface.GetAddress (0));
        }
      }
      else
      {
        ...
        //Connect all of numbers client nodes to the UL switch.
        NetDeviceContainer ul2clLink = csmaHelper.Install (switchNodeUl, clientNodes.Get (i));
        uint32_t ul2clPort = switchDeviceUl->AddSwitchPort (ul2clLink.Get (0))->GetPortNo ();
        clientDevices.Add (ul2clLink.Get (1));
  ...
  Ptr<TrafficHelper> trafficHelper =
    CreateObject<TrafficHelper> (controllerApp, serverNodes, clientNodes);
  ...
  // HW
  uint16_t port = 7000;
  UdpPingServerHelper serverHW (port);
  ApplicationContainer appsHW = serverHW.Install (pingServerNode);
  // SW
  port = 7001;
  UdpPingServerHelper serverSW (port);
  ApplicationContainer appsSW = serverSW.Install (pingServerNode);
  //
  ...
  UdpPingClientHelper clientHW (pingServerIface.GetAddress (0), 7000); //Server Port
  clientHW.SetAttribute ("Interval", TimeValue (interPacketInterval));
  clientHW.SetAttribute ("PacketSize", UintegerValue (packetSize));
  clientHW.SetAttribute ("Port", UintegerValue (65534));
  appsHW = clientHW.Install (pingClientNode);
  appsHW.Start (Seconds (1.0));
  appsHW.Stop (Seconds (simTime-1));
  // SW
  UdpPingClientHelper clientSW (pingServerIface.GetAddress (0), 7001); //Server Port
  clientSW.SetAttribute ("Interval", TimeValue (interPacketInterval));
  clientSW.SetAttribute ("PacketSize", UintegerValue (packetSize));
  clientSW.SetAttribute ("Port", UintegerValue (65535));
  appsSW = clientSW.Install (pingClientNode);
  appsSW.Start (Seconds (1.0));
  appsSW.Stop (Seconds (simTime-1));
  ...
  if (pcap)
    {
      ...
      csmaHelper.EnablePcap (outPrefix + "server", serverDevices,  true);
      csmaHelper.EnablePcap (outPrefix + "switch", switchNodes,   true);
      csmaHelper.EnablePcap (outPrefix + "pingSv", pingServerLink,   true);
      csmaHelper.EnablePcap (outPrefix + "pingCl", pingClientLink,   true);
    }
  ...
  Simulator::Run ();
  controllerApp->PrintTrafficInfo();
  Simulator::Destroy ();
  ...
  if (enable)
    {
      ...
      //LogComponentEnable ("TrafficStatistics",        logLevelInfo);

      // Applications.
      //LogComponentEnable ("AppStatsCalculator",       logLevelInfo);
      //LogComponentEnable ("SvelteClient",             logLevelInfo);
      //LogComponentEnable ("SvelteServer",             logLevelInfo);
      //LogComponentEnable ("SvelteUdpClient",          logLevelInfo);
      //LogComponentEnable ("SvelteUdpServer",          logLevelInfo);
      //LogComponentEnable ("UdpPingServerApplication",   logLevelInfo);
      //LogComponentEnable ("UdpPingClientApplication",   logLevelInfo);
