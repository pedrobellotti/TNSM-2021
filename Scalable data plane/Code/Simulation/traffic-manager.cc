...
TrafficManager::AddSvelteClient (Ptr<SvelteClient> app)
{
  ...
  Time firstTry = Seconds (std::abs (m_poissonRng->GetValue ()));
  m_count += firstTry;
  Simulator::Schedule (m_count, &TrafficManager::AppStartTry, this, app);
  NS_LOG_INFO ("First start attempt for app " << app->GetAppName () <<
               " will occur at " << m_count.GetSeconds () << "s.");
}

void
TrafficManager::SetController (Ptr<QosCustomController> controller)
{
...
TrafficManager::AppStartTry (Ptr<SvelteClient> app)
{
  ...
  if (now >= 200.0 && now <= 400.0 && val > 90 && m_pingTrackedApps < 125)
  {
    app->SetPingTracking();
    m_pingTrackedApps += 1;
  }
