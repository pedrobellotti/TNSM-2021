...
public:
   ...
   */
  void SetController (Ptr<QosCustomController> controller);
...
private:
  ...
  Ptr<QosCustomController>  m_ctrlApp;          //!< OpenFlow slice controller.
  uint64_t                  m_imsi;             //!< UE IMSI identifier.
  uint64_t                  m_defaultTeid;      //!< Default UE tunnel TEID.
  Time                      m_count;            //!< Timer count.
  uint64_t                  m_pingTrackedApps;  //!< Ping tracked apps counter.
