...
void
AppStatsCalculator::NotifyRx (uint32_t rxBytes, std::string cookie, bool trackPing, Time timestamp)
{
  ...
  if (trackPing){
    SimulationSingleton<PingStatsFile>::Get()->SaveToFile(Simulator::Now ().GetSeconds (), cookie, now-timestamp, GetRxJitter());
  }
}
