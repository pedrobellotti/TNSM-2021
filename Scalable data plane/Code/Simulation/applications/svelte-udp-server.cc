...
SvelteUdpServer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::SvelteUdpServer")
    ...
    .AddAttribute ("PktSize", "The size of packets sent in on state",
                    UintegerValue (512),
                    MakeUintegerAccessor (&SvelteUdpServer::m_pktSize),
...
void
SvelteUdpServer::SendOnePacket ()
{
  NS_LOG_FUNCTION (this);

  Ptr<Packet> packet = Create<Packet> (m_pktSize); //Erro aqui
...
SvelteUdpServer::ReadPacket (Ptr<Socket> socket)
{
  ...
  NotifyRx (packet->GetSize (), GetTeidHex(), m_pingTracking, seqTs.GetTs ());
