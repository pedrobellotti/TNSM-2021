...
public:
  ...
  uint64_t                      GetTeid           (void) const;
  std::string                   GetTeidHex        (void) const;
  bool                          GetPingTracking   (void) const;
...
protected:
   ...
   */
  void NotifyRx (uint32_t rxBytes, std::string cookie, bool track, Time timestamp = Simulator::Now ());
