...
void
SvelteClient::SetPingTracking ()
{
  NS_LOG_FUNCTION (this);
  m_trackPing = true;
  m_serverApp->SetPingTracking(GetTeidHex());
  //std::cout << "Client " << GetTeidHex() << " enviou settracking" << std::endl;
}

bool
SvelteClient::GetPingTracking () const
{
  NS_LOG_FUNCTION (this);
  return m_trackPing;
}
...
void
SvelteClient::SetTeid (uint64_t value)
{
  NS_LOG_FUNCTION (this << value);
...
void
SvelteClient::NotifyRx (uint32_t rxBytes, std::string cookie, bool track, Time timestamp)
{
  NS_LOG_FUNCTION (this << rxBytes << timestamp);
  m_appStats->NotifyRx (rxBytes, GetTeidHex(), track, timestamp);
}
